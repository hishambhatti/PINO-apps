...
class BurgersEq2D_Vec():
    ...
    def burgers_calc_RHS(self, u, v):
        ...
        # u2_y = self.Dy(u2)
        # u_RHS = -0.5*(u2_x + u2_y) + self.nu*(u_xx + u_yy)
        u_RHS = -u*u_x - v*u_y + self.nu*(u_xx + u_yy)
        v_RHS = -u*v_x - v*v_y + self.nu*(v_xx + v_yy)
        return u_RHS, v_RHS
    ...
    def burgers_rk4(self, u, v, t=0):
        u_RHS1, v_RHS1 = self.burgers_calc_RHS(u, v)
        t1 = t + 0.5*self.dt
#         display(u.shape)
#         display(u_RHS1.shape)
        u1 = self.update_field(u, u_RHS1, step_frac=0.5)
        ...
        u_RHS2, v_RHS2 = self.burgers_calc_RHS(u1, v1)
        t2 = t + 0.5*self.dt
        u2 = self.update_field(u, u_RHS2, step_frac=0.5)
        v2 = self.update_field(v, v_RHS2, step_frac=0.5)
        
        u_RHS3, v_RHS3 = self.burgers_calc_RHS(u2, v2)
        t3 = t + self.dt
        u3 = self.update_field(u, u_RHS3, step_frac=1.0)
        v3 = self.update_field(v, v_RHS3, step_frac=1.0)
        
        u_RHS4, v_RHS4 = self.burgers_calc_RHS(u3, v3)
    ...
    def burgers_driver(self, u0, v0, save_interval=10, plot_interval=0):
        # plot results
        # t,it = get_time(time)
#         display(u0[:self.Nx,:self.Ny].shape)
        # self.u0 = u0[:self.Nx]
        ...
        while self.t < self.tend:
#             print(f"t:\t{self.t}")
            self.u, self.v, self.t = self.burgers_rk4(self.u, self.v, self.t)
        ...
        return torch.stack(self.U), torch.stack(self.V)
